...
type server struct {
	...
	pqRead map[int]*PriorityQueue //88
	...
	for {
		...
		case messagewithUDP := <-s.serverMessageChannel:
			...
			if msg.Type == MsgConnect {
				...
				s.pqRead[currentConnID_toClient] = NewPriorityQueue()
				...
				curReadSeqNum := s.currentReadSeqNum[msg.ConnID]
				//<-s.currentReadSeqNumLock
				if msg.SeqNum >= (curReadSeqNum + 1) {
					s.pqRead[msg.ConnID].Insert(&newPqReadMessage, float64(msg.SeqNum))
				...
				s.pqReadLock <- struct{}{}
				curPqReadTop := s.pqRead[msg.ConnID].Top().(*Message)
				log.Print("In msgData: server get messgae seqnum: ", msg.SeqNum, " with cureadsqenum: ", curReadSeqNum,
					" and pqreadtopseqnum: ", curPqReadTop.SeqNum)
				for (curReadSeqNum + 1) == (curPqReadTop.SeqNum) {
					s.pqRead[msg.ConnID].Pop()
					...
					if s.pqRead[msg.ConnID].Len() > 0 {
						curPqReadTop1 := s.pqRead[msg.ConnID].Top().(*Message)
						log.Print(curPqReadTop1)
						curReadSeqNum = curReadSeqNum + 1
						for s.pqRead[msg.ConnID].Len() > 0 {
							if s.pqRead[msg.ConnID].Top().(*Message).SeqNum <= curReadSeqNum {
								s.pqRead[msg.ConnID].Pop()
							} else {
								break
							}
