...
func (s *server) readRoutine() {
	for {
		...
		default:
			...
			log.Print("server get message with type: ", msg.message.Type, " and seqnum: ", msg.message.SeqNum)
		}
	}
}
...
func (s *server) writeRoutine() {
	for {
		...
		case msg := <-s.writeChan:
			...
			log.Print("server do write message with type: ", msg.message.Type, " and seqnum: ", msg.message.SeqNum)
		case <-s.closeWrite:
			return
		}
	}
